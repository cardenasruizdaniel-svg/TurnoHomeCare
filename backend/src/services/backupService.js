...
class BackupService {
   ...
   */
  static async getDatabaseBuffer() {
    if (db.persistToDisk) db.persistToDisk();
    const currentPath = this.getDbPath();
   ...
   */
  static async restoreFromBuffer(buffer) {
    const currentPath = this.getDbPath();
    const backupDir = path.join(path.dirname(currentPath), 'backups');
    if (!fs.existsSync(backupDir)) {
      fs.mkdirSync(backupDir, { recursive: true });
   ...
   */
  static async exportFullDataJson() {
    const tables = [
   ...
   */
  static async importFullDataJson(jsonData) {
    ...
    const transaction = db.transaction(async () => {
      ...
      if (Array.isArray(database.branches)) {
        for (const b of database.branches) {
          const exists = await db.prepare('SELECT id FROM branches WHERE id = ?').get(b.id);
          if (exists) {
            await db.prepare(`
              UPDATE branches 
      ...
      if (Array.isArray(database.services)) {
        for (const s of database.services) {
          const exists = await db.prepare('SELECT id FROM services WHERE id = ?').get(s.id);
          if (exists) {
            await db.prepare(`
              UPDATE services 
      ...
      if (Array.isArray(database.counters)) {
        for (const c of database.counters) {
          const exists = await db.prepare('SELECT id FROM counters WHERE id = ?').get(c.id);
          if (exists) {
            await db.prepare(`
              UPDATE counters 
      ...
      if (Array.isArray(database.settings)) {
        for (const set of database.settings) {
          const exists = await db.prepare('SELECT id FROM settings WHERE key = ? AND (branch_id = ? OR (branch_id IS NULL AND ? IS NULL))').get(set.key, set.branch_id, set.branch_id);
          if (exists) {
