...
export function AdminSettingsView() {
  ...
  const sanitizePrimitive = (v, defaultVal = '') => {
    ...
    if (typeof v === 'object' && v.value !== undefined) return sanitizePrimitive(v.value, defaultVal);
    if (typeof v === 'object') return defaultVal;
    return v;
  };
  ...
  const loadSettings = async () => {
      ...
      if (res && res.success) {
        if (res.company && typeof res.company === 'object') {
          setCompany({
            ...
            secondary_color: sanitizePrimitive(res.company.secondary_color, '#00b0b9'),
            accent_color: sanitizePrimitive(res.company.accent_color, '#7cb518')
          });
        }
        if (res.settings && typeof res.settings === 'object') {
          const map = {};
          Object.keys(res.settings).forEach(k => {
            let item = res.settings[k];
            let val = (item && typeof item === 'object' && item.value !== undefined) ? item.value : item;
            if (k === 'BANNERS_PUBLICIDAD') {
              if (typeof val === 'string') {
                try { val = JSON.parse(val); } catch { val = []; }
              }
              if (!Array.isArray(val)) val = [];
              map[k] = val.filter(b => b && typeof b === 'object').map((b, i) => ({
                id: sanitizePrimitive(b.id, `b_${i}`),
                title: sanitizePrimitive(b.title, ''),
                subtitle: sanitizePrimitive(b.subtitle, ''),
                tag: sanitizePrimitive(b.tag, ''),
                imageUrl: sanitizePrimitive(b.imageUrl, ''),
                videoUrl: sanitizePrimitive(b.videoUrl, ''),
                mediaType: sanitizePrimitive(b.mediaType, 'image'),
                isActive: b.isActive !== false
              }));
            } else {
              map[k] = sanitizePrimitive(val, '');
            }
          });
    ...
    try {
      ...
      if (tRes && tRes.success) {
        setTunnelStatus({
          active: !!tRes.active,
          public_url: sanitizePrimitive(tRes.public_url, null),
          local_url: sanitizePrimitive(tRes.local_url, 'http://127.0.0.1:5000')
        });
      }
    } catch (e) {
                    ...
                    <div className="space-y-2 text-xs">
                      ...
                      <div>
                        <label className="text-[10px] text-slate-500 font-bold block">
                          URL de Imagen o Video (.mp4 / .webm / .ogg)
                        </label>
                        <input
                          type="text"
                          ...
                          onChange={(e) => {
                            ...
                            const isVid = val.includes('.mp4') || val.includes('.webm') || val.includes('.ogg') || val.includes('.mov') || val.startsWith('data:video/');
                            const currentBanners = Array.isArray(settings.BANNERS_PUBLICIDAD) ? [...settings.BANNERS_PUBLICIDAD] : [];
                            if (currentBanners[idx]) {
                              currentBanners[idx] = {
                                ...currentBanners[idx],
                                mediaType: isVid ? 'video' : (currentBanners[idx].mediaType || 'image'),
                                imageUrl: val,
                                videoUrl: isVid ? val : (currentBanners[idx].videoUrl || '')
                              };
                              setSettings(prev => ({ ...prev, BANNERS_PUBLICIDAD: currentBanners }));
                            }
                          }}
