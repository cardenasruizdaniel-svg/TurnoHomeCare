...
export function StaffDeskView() {
  ...
  const [selectedCounterId, setSelectedCounterId] = useState(() => {
    ...
    const saved = localStorage.getItem(`deaturnos_staff_counter_id_branch_${bId}`);
  ...
  useEffect(() => {
    ...
    api.getCounters(branchId).then(res => {
      if (res.success && res.counters) {
        ...
        const saved = localStorage.getItem(`deaturnos_staff_counter_id_branch_${branchId}`);
        const savedId = saved ? Number(saved) : null;
        const exists = res.counters.find(c => c.id === (savedId || selectedCounterId));
        if (exists) {
          setSelectedCounterId(exists.id);
        } else if (res.counters.length > 0) {
  ...
  // Guardar módulo en localStorage por sede
  useEffect(() => {
    if (selectedCounterId && branchId) {
      localStorage.setItem(`deaturnos_staff_counter_id_branch_${branchId}`, String(selectedCounterId));
      loadQueueAndStatus();
    }
  }, [selectedCounterId, branchId]);
